...
class Response {
  ...
  async errorObjGenerator(error) {
    ...
    if (error.original) {
      switch (error.original.code) {
        case "ER_DUP_ENTRY":
          let resp = await errorCodeHandlers[error.original.code](
            error.original
          );
  ...
  errorsWithOutReq(status, message) {
    ...
    let msg = message;
    msg = codeMsgs[status] ? codeMsgs[status] : "Internel server error!";
    this.createLogs({ status, req, msg });
